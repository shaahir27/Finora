...
- **No rate limiting** on Gemini-backed endpoints (`processOcrUpload`, `answerDashboardQuery`), `generateReconciliationReport` *(added Phase 14 — same rate-limit scope, see `system_architecture.md` Rate Limiting / Cost Control)*, or file uploads. At current build scale the realistic abuse risk is low, but this is not solved — see `system_architecture.md` Future Extensions.
